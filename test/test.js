...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dsort, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function sorts a one-dimensional ndarray (increasing order)', function test( t ) {
	...
	x = ndarray( 'float64', new Float64Array( [ 3.0, 1.0, 2.0, 5.0, 4.0 ] ), [ 5 ], [ 1 ], 0, 'row-major' );
	order = scalar2ndarray( 1.0, {
		'dtype': 'generic'
	});

	actual = dsort( [ x, order ] );
